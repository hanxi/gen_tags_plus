...
"----------------------------------------------------------------------
" search gtags db by gen_tags
"----------------------------------------------------------------------
function! s:get_gtags_file() abort
    let tags = gen_tags#get_db_dir().'/GTAGS'
...
function! s:GscopeAdd() abort
    ...
    let root = gen_tags#get_db_dir()
    if dbname == '' || root == ''
        call s:ErrorMsg("no gtags database for this project, check g_tags's documents")
        return 0
    endif
